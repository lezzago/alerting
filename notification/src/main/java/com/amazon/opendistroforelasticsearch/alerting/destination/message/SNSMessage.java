...
public class SNSMessage extends BaseMessage {
    ...
    private SNSMessage(final DestinationType destinationType, final String destinationName, final String roleArn,
        ...
        if (!SNS_ODFE_SUPPORT && (Strings.isNullOrEmpty(roleArn) || !Util.isValidIAMArn(roleArn))) {
            throw new IllegalArgumentException("Role arn is missing/invalid: " + roleArn);
        }

        if (SNS_ODFE_SUPPORT && (iamAccessKey == null || Strings.isNullOrEmpty(iamAccessKey.toString()))) {
            throw new IllegalArgumentException("IAM user access key is missing");
        }

        if (SNS_ODFE_SUPPORT && (iamSecretKey == null || Strings.isNullOrEmpty(iamSecretKey.toString()))) {
    ...
    public String toString() {
        String snsAccess = SNS_ODFE_SUPPORT ? String.format(", AccessKey: %s, SecretKey: %s", iamAccessKey, iamSecretKey)
                : String.format(", RoleArn: %s", roleArn);
        return "DestinationType: " + destinationType + ", DestinationName:" +  destinationName +
                snsAccess + ", TopicArn: " + topicArn + ", Subject: " + subject +
                ", Message: " + message;
    }
